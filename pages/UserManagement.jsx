...
export default function UserManagement() {
  ...
  const updateUserMutation = useMutation({
    ...
    onSuccess: async (updatedUser, variables) => {
      ...
      // Create a system log signaling CRM sync intent
  ...
  const revokeAccessMutation = useMutation({
    mutationFn: async ({ userId, reason }) => {
      // Update user to revoked/inactive status
      await apiClient.update('User', userId, {
        role: "revoked"
      });
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    if (targetUser?.role === "admin" && newRole !== "admin" && adminCount === 1) {
      alert("Cannot change the role of the last admin user. There must always be at least one admin.");
      return;
    }
  ...
  const copyInviteLink = (invite) => {
    const link = `${config.clientPortalUrl}/PortalRegister?token=${invite.invite_token}`;
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
            ...
            <Card className="p-4 border-0 shadow-lg">
              <div className="text-2xl font-bold text-purple-600">
                {clientInvites.filter(i => i.used).length}
              </div>
              <div className="text-sm text-gray-600">Client Accounts</div>
            </Card>
            <Card className="p-4 border-0 shadow-lg">
              <div className="text-2xl font-bold text-green-600">
                {users.filter(u => u.created_date && 
                      ...
                      <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <Badge className={`${roleColors[user.role] || roleColors.user} border`}>
                              {user.role}
                            </Badge>
        ...
        <Dialog open={showInviteClientDialog} onOpenChange={setShowInviteClientDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Send an invitation to join the client portal at clientportal.pacificengineeringsf.com
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button variant="outline" onClick={() => setShowInviteClientDialog(false)}>
                  Cancel
                </Button>
                <Button
                  onClick={() => inviteClientMutation.mutate({ 
        ...
        <Dialog open={showInviteDialog} onOpenChange={setShowInviteDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Send an invitation to join the internal portal at internalportal.pacificengineeringsf.com
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Configure role and granular permissions for {editingUser?.full_name}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-6 py-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Admin role automatically grants all permissions. User role requires manual permission assignment below.
                  </p>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      onClick={async () => {
                        // Manual sync action to CRM (internal)
                        await apiClient.create('SystemLog', {
                          type: 'user_permissions_sync',
                          user_id: editingUser.id,
                          user_email: editingUser.email,
                          role: editingUser.role,
                          permissions: editingUser.role === "admin" ? [] : editPermissions,
                          status: 'synced',
                          created_at: new Date().toISOString()
                        });
                        await apiClient.create('AuditLog', {
                          ...
                          resource_name: editingUser.full_name,
                          details: 'Synced user role/permissions to CRM'
                        });
